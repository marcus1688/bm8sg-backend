...
const live22OperatorID = "l22bm8sgSGD";
  ...
  try {
    ...
    return res.status(200).json({
      ...
      message: {
        ...
        zh_hk: "LIVE22: 無法獲取遊戲列表，請聯絡客服以獲取幫助。",
...
router.post("/api/live22/launchGame", authenticateToken, async (req, res) => {
  try {
    ...
    const { gameCode, gameLang, gameType } = req.body;
    ...
    const isLocked =
      gameType === "Arcade"
        ? user.gameLock?.live22other?.lock
        : user.gameLock?.live22?.lock;
    ...
    if (response.data.Description !== "Success") {
      ...
      return res.status(200).json({
        ...
        message: {
          ...
          zh: "LIVE22: 游戏启动失败，请重试或联系客服以获得帮助。",
          ms: "LIVE22: Pelancaran permainan gagal. Sila cuba lagi atau hubungi khidmat pelanggan untuk bantuan.",
          zh_hk: "LIVE22: 遊戲啟動失敗，請重試或聯絡客服以獲得幫助。",
...
router.post("/api/live22sg/GetBalance", async (req, res) => {
  try {
...
router.post("/api/live22sg/Bet", async (req, res) => {
  try {
    const {
      OperatorId,
      Signature,
...
router.post("/api/live22sg/GameResult", async (req, res) => {
  try {
    const {
      OperatorId,
      Signature,
...
router.post("/api/live22sg/Rollback", async (req, res) => {
  try {
    const {
      OperatorId,
      Signature,
...
router.post("/api/live22sg/CashBonus", async (req, res) => {
  try {
...
router.post("/api/live22sg/BetNSettle", async (req, res) => {
  try {
    const {
      OperatorId,
      Signature,
    ...
    const [currentUser, existingTransaction] = await Promise.all([
      ...
      SlotLive22Modal.exists({ betId: BetId }),
    ]);
    ...
    const betAmount = new Decimal(Number(BetAmount)).toDecimalPlaces(4);
    const payout = new Decimal(Number(Payout)).toDecimalPlaces(4);
    const netWin = payout.minus(betAmount).toDecimalPlaces(4);
    ...
    const updatedUser = await User.findOneAndUpdate(
      { gameId: PlayerId, wallet: { $gte: betAmount.toNumber() } },
      { $inc: { wallet: netWin.toNumber() } },
      { new: true, projection: { wallet: 1 } }
    ).lean();
    ...
    await SlotLive22Modal.create({
      ...
      settle: true,
      settleamount: payout.toNumber(),
      gametype,
    });

    const newBalance = new Decimal(Number(updatedUser.wallet)).toDecimalPlaces(
      4
    );
    ...
    console.error(
      "LIVE22: Error in game provider calling betnsettle api:",
      error
    );
    return res.status(200).json({
      Status: 900500,
...
router.post("/api/live22sg/CancelBetNSettle", async (req, res) => {
  try {
    const {
      OperatorId,
      Signature,
    ...
    const [currentUser, existingTransaction] = await Promise.all([
      User.findOne({ gameId: PlayerId }, { wallet: 1 }).lean(),
      SlotLive22Modal.findOne({ betId: BetId }, { _id: 1 }).lean(),
    ]);
    ...
    if (!existingTransaction) {
      return res.status(200).json({
        Status: 900417,
        Description: "CancelBetNSettle Rejected",
        ResponseDateTime: RequestDateTime,
        OldBalance: oldBalance.toNumber(),
        NewBalance: oldBalance.toNumber(),
      });
    }
    ...
    console.error(
      "LIVE22: Error in game provider calling cancel betn settle api:",
      error.message
    );
    return res.status(200).json({
      Status: 900500,
...
router.post("/api/live22other/getturnoverforrebate", async (req, res) => {
  try {
    const { date } = req.body;
    ...
    console.log("LIVE22 OTHER QUERYING TIME", startDate, endDate);
...
router.get(
  "/admin/api/live22other/:userId/dailygamedata",
  authenticateAdminToken,
  async (req, res) => {
    try {
      const { startDate, endDate } = req.query;
...
router.get(
  "/admin/api/live22other/:userId/gamedata",
  authenticateAdminToken,
  async (req, res) => {
    try {
      const { startDate, endDate } = req.query;
...
router.get(
  "/admin/api/live22other/dailykioskreport",
  authenticateAdminToken,
  async (req, res) => {
    try {
      const { startDate, endDate } = req.query;
...
router.get(
  "/admin/api/live22other/kioskreport",
  authenticateAdminToken,
  async (req, res) => {
    try {
      const { startDate, endDate } = req.query;
