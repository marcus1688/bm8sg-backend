...
const cmdWebLaunchGameUrl = "https://egegm.fts368.com";
const cmdMobileLaunchGameUrl = "https://egegmsmart.fts368.com";
...
async function registerCMDUser(username) {
  try {
    const response = await axios.get(
      `${cmdAPIURL}/?Method=createmember&PartnerKey=${cmdPartnerKey}&UserName=${username}&Currency=SGD`,
...
router.get("/api/cmd368prod/auth", async (req, res) => {
  try {
...
router.get("/api/cmd368prod/getbalance", async (req, res) => {
  try {
...
router.post("/api/cmd368prod/deductbalance", async (req, res) => {
  try {
...
router.post("/api/cmd368prod/updatebalance", async (req, res) => {
  try {
    const { balancePackage, method, packageId, dateSent } = req.body;
